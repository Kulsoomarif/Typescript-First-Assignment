...
let current_users = ["Sara", "saba", "Sana", "sania", "Salma"];
let new_users = ["Ali", "Saba", "Ani", "Sania", "Adil"];
new_users.forEach(new_one => {
    let condition = current_users.some(current_one_user => current_one_user.toLowerCase() === new_one.toLowerCase());
    if (condition) {
        console.log(`sorry!!! ${new_one} is already taken.`);
    }
    else {
        console.log(`This username ${new_one} is available.`);
    }
});
